    ...
    public List<Customer> getbookList() {
        ...
        try{
            ...
            while(rs.next()){
                ...
                cs.setBookingStatus(rs.getString("status"));
...
public void bookaccept( Customer cd) throws SQLException {
    ...
    try (PreparedStatement pstm = new DBConnection().getStatement(query)) {
        ...
        pstm.setString(1, cd.getBookingStatus());
        pstm.setInt(2, cd.getBookingid());
        pstm.executeUpdate();
        
    }
    
}
public void bookreject( Customer cd) throws SQLException {
