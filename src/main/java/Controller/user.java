...
//        Show the user list in list user page
        ...
        if (test.equalsIgnoreCase("listvehicle")) {
            Student student = new Student();
            List<Student> RoomList = new UserService().getRoomList();
            request.setAttribute("student", student);
            request.setAttribute("RoomList", RoomList);
            RequestDispatcher rd = request.getRequestDispatcher("Pages/vehiclelist.jsp");
...
//        For user_details
        ...
        if (test.equalsIgnoreCase("vehicleDetails")) {
            int id = Integer.parseInt(request.getParameter("id"));
            Student student = new UserService().getRoomRow(id);
            request.setAttribute("id", id);
            request.setAttribute("student", student);

            RequestDispatcher rd = request.getRequestDispatcher("Pages/vehicle_details.jsp");
            rd.forward(request, response);
        }
...
// for deleting room
        if (test.equalsIgnoreCase("deletevehicle")) {
            int id = Integer.parseInt(request.getParameter("id"));
            UserService userService = new UserService();
...
// for editing users
        ...
        if (test.equalsIgnoreCase("editvehicle")) {
            int id = Integer.parseInt(request.getParameter("id"));
            Student student = new UserService().getRoomRow(id);
            request.setAttribute("id", id);
            request.setAttribute("student", student);
            RequestDispatcher rd = request.getRequestDispatcher("Pages/update_vehicle.jsp");
            rd.forward(request, response);
        }
        ...
        if (test.equalsIgnoreCase("addvehicle")) {

            Part filePart = request.getPart("file");
            String fileName = filePart.getSubmittedFileName();
            String uploadPath = "C:\\Users\\ajayb\\Desktop\\Nikku\\AAAAAAA\\src\\main\\webapp\\image\\upload\\pics";
        ...
        // to accept the user booking 
        
        if (test.equalsIgnoreCase("accept")) {
            
            Customer cd = new Customer();
            
             cd.setBookingid(Integer.parseInt(request.getParameter("id")));
             cd.setBookingStatus(request.getParameter("value"));
             
            try {
                new UserService().bookaccept(cd);
        ...
        if (test.equalsIgnoreCase("reject")) {
