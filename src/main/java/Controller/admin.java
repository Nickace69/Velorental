...
//                    System.out.println(session.getAttribute("user"));
        ...
        if (practice.equalsIgnoreCase("changingpassword")) {
            ...
            customer.setPassword(HashPassword.hashPassword(request.getParameter("oldpassword")));
            customer.setNewpassword(HashPassword.hashPassword(request.getParameter("newpassword")));

            new CustomerService().changePassword(customer, username);
...
//           
        ...
        if (practice.equalsIgnoreCase("listvehicle")) {
        ...
        if (practice.equalsIgnoreCase("rsearch")) {
            String query = request.getParameter("query");
            List<Student> roomsearch = UserService.searchrooms(query);
            request.setAttribute("roomsearch", roomsearch);
            request.setAttribute("query", query);
            RequestDispatcher dispatcher = request.getRequestDispatcher("CustomerPage/vehiclesearchresult.jsp");
            dispatcher.forward(request, response);
        }
        
        if(practice.equalsIgnoreCase("logout")){
         ...
         if (practice != null && practice.equalsIgnoreCase("vehiclesdetails")) {
            Student student = new Student();
            List<Student> RoomLiist = new UserService().getRoomList();
            request.setAttribute("student", student);
            request.setAttribute("RoomLiist", RoomLiist);
            RequestDispatcher rd = request.getRequestDispatcher("CustomerPage/vehicledetails.jsp");
            ...
            customer.setBookingStatus(request.getParameter("status"));

            new CustomerService().insertBoking(customer);

            RequestDispatcher rd = request.getRequestDispatcher("CustomerPage/bookingstatus.jsp");
            try {
                rd.forward(request, response);
            } catch (ServletException e) {
                throw new RuntimeException(e);
         ...
         if (practice.equalsIgnoreCase("listbooking")) {
            Customer customer = new Customer();
